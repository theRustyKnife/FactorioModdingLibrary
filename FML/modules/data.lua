...
function M.inherit(base_type, base_name)
	...
	return data.raw[base_type][base_name]
end
...
function M.make_prototypes(prototypes)
	for _, p in ipairs(prototypes) do M.make_prototype(p); end
end

function M.make_prototype(prototype)
...
function M.is_result(recipe, item)
